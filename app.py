...
with st.container():
    ...
    p_disease = st.text_input("疾患名", placeholder="例：右被殻出血、大腿骨近位部骨折")
    p_symptom = st.text_input("主な症状", 
                           placeholder="例：弛緩性麻痺、半側空間無視")
    p_severity = st.text_input("重症度（任意）", placeholder="例：SIAS 30点、座位保持困難、BRS：Ⅲ")

    st.subheader("💡 介入（I）・アウトカム（O）")
    i_input = st.text_input("介入 (任意): 具体的な技術（空欄で最新トレンド検索）", 
                          placeholder="例：装具療法、エルゴメーター")
